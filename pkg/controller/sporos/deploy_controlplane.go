...
func (r *ReconcileSporos) createExternalEndpoint(cr *api.Sporos) (*corev1.Service, error) {
...
func (r *ReconcileSporos) isServiceEndpointReady(cr *api.Sporos, s *corev1.Service) (bool, error) {
	err := r.client.Get(context.TODO(), types.NamespacedName{Name: s.GetName(), Namespace: s.GetNamespace()}, s)
	if err != nil {
		return false, err
	}
	...
	log.Info("API server endpoint: %s", cr.Status.ApiServerIP)
	return true, nil
}

func (r *ReconcileSporos) deployControlplane(cr *api.Sporos) ([]*appsv1.Deployment, error) {
	var deployments []*appsv1.Deployment

	apiServerSecret := fmt.Sprintf("%s-kube-apiserver", cr.GetName())
	apiServerName := fmt.Sprintf("%s-kube-apiserver", cr.GetName())
	api, err := r.createDeployment(cr, apiServerName, apiServerSecret, r.apiserverContainer)
	if err != nil {
		return nil, err
	}
	...
	control, err := r.createDeployment(cr, controllerName, controllerSecret, r.controllerContainer)
	if err != nil {
		return nil, err
	}
	...
	scheduler, err := r.createDeployment(cr, schedulerName, schedulerSecret, r.schedulerContainer)
	if err != nil {
		return nil, err
	}
...
func (r *ReconcileSporos) createDeployment(cr *api.Sporos, name, secretName string, containerfn func(*api.Sporos) corev1.Container) (*appsv1.Deployment, error) {
...
func (r *ReconcileSporos) apiserverContainer(cr *api.Sporos) corev1.Container {
...
func (r *ReconcileSporos) controllerContainer(cr *api.Sporos) corev1.Container {
	return corev1.Container{
...
func (r *ReconcileSporos) schedulerContainer(cr *api.Sporos) corev1.Container {
	return corev1.Container{
		Name:  "kube-scheduler",
		Image: fmt.Sprintf("%s:%s", cr.Spec.BaseImage, cr.Spec.Version),
		Command: []string{
...
func (r *ReconcileSporos) IsControlplaneReady(d *appsv1.Deployment) (bool, error) {
	err := r.client.Get(context.TODO(), types.NamespacedName{Name: d.GetName(), Namespace: d.GetNamespace()}, d)
	if err != nil {
		return false, err
	}

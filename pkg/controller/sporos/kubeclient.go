...
func (r *ReconcileSporos) NewKubeClient(cr *api.Sporos) (*kubernetes.Clientset, error) {
	...
	err := r.client.Get(context.TODO(), types.NamespacedName{Name: adminSecret.GetName(), Namespace: adminSecret.GetNamespace()}, adminSecret)
	if err != nil {
		return nil, err
	}
	...
	upFn := func() (bool, error) {
		if err := apiTest(clientset); err != nil {
			log.Info("Unable to determine api-server readiness: %v", err)
			return false, nil
		}
		return true, nil
	}
